...
def img_mask_ori_spinal_neural(img,mask):
...
def model_plot_spinal_neural(model, test_dataset, list_img):
...
def img_mask_ori_subarticular(img,mask):
    # Get the image and mask
    #img, mask = spinal_train_dataset[200]
...
def model_plot_subarticular (model, test_dataset, list_img):
    f, ax = plt.subplots(2, 4, figsize=(13, 8))
